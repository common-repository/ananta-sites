 ...
 */
function ananta_post_blog_2_render_callback($attributes) {
    $catarr = [];
	...
	<div class="ananta-block-container-<?php echo esc_attr( $attributes['yourId'] ); ?> <?php echo esc_attr( $attributes['hideDesktop'] ? 'hidden-desktop' : '' ); ?> <?php echo esc_attr( $attributes['hideTablet'] ? 'hidden-tablet' : '' ); ?> <?php echo esc_attr( $attributes['hideMobile'] ? 'hidden-mobile' : '' ); ?>">
		<div class="anb-blog-wrapper-<?php echo $attributes['yourId']; ?> anb-blog-grid anb-grid-one anb-column-<?php echo $attributes['columns']; ?>">
			<?php
			if ($post_query->have_posts()) :
				while ($post_query->have_posts()) :
					...
					?>
				<div class="anb-blog-post-overlay layout-one ananta-blog-card <?php echo $attributes['categoryPosition']; ?>">
 ...
 */
function ananta_post_blog_2_featured_images_restapi( $data, $post, $context ) {
  ...
  add_filter( 'rest_prepare_post', 'ananta_post_blog_2_featured_images_restapi', 10, 3 );

  function ananta_blocks_post_2_register_rest_fields() {
    ...
    register_rest_field(
        ...
        'author_info',
        array(
            'get_callback'    => 'ananta_post_2_blocks_get_author_info',
...
add_action( 'rest_api_init', 'ananta_blocks_post_2_register_rest_fields' );

  function ananta_post_2_blocks_get_author_info( $object,  $field_name, $request ) {
    /* Get the author name */
